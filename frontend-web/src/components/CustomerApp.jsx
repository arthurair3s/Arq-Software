...
import { GET_RESTAURANTES, CRIAR_PEDIDO, ACOMPANHAR_PEDIDO } from '../graphql/queries';
...
export default function CustomerApp({ activePedidoId, setActivePedidoId }) {
  ...
  const { data: trackData, startPolling, stopPolling } = useQuery(ACOMPANHAR_PEDIDO, {
  ...
  return (
    <div className="glass-panel animate-fade-in">
      <div className="panel-header">
        ...
        {pedido && (
          <span className={`status-badge ${entrega?.status?.toLowerCase() || 'pendente'}`}>
      ...
      <div className="panel-content">
        {!activePedidoId ? (
          <>
            <p className="text-secondary mb-4">Escolha um restaurante da lista e faça seu pedido:</p>
            ...
            <div className="grid" style={{ display: 'grid', gap: '16px' }}>
              {restData?.restaurantes?.map(r => (
                <div key={r.id} className="glass-card card">
                  ...
                  <p className="card-text mb-4 text-muted"><MapPin size={14} className="inline mr-1"/>{r.endereco}</p>
                  <button 
          ...
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div className="glass-card" style={{ padding: '16px' }}>
              ...
              <p className="text-muted">Pedido #{pedido?.id} | Entregador: <strong className="text-primary">{entrega?.entregador?.nome || 'Procurando...'}</strong></p>
              
              {resumo && (
                <div style={{ display: 'flex', gap: '20px', marginTop: '12px' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <Navigation size={18} className="text-accent"/> {resumo.distancia_km.toFixed(1)} km
                  </span>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <Clock size={18} className="text-warning"/> {Math.ceil(resumo.duracao_estimada_segundos / 60)} min
                  </span>
                </div>
              )}
            </div>

            <div style={{ flex: 1, position: 'relative', borderRadius: '16px', overflow: 'hidden', minHeight: '350px' }}>
              {(rota.length > 0) ? (
                <MapContainer 
                ...
                >
                  ...
                  <Polyline 
                    ...
                    color={entrega?.status === 'EM_TRANSITO' ? '#10b981' : '#f59e0b'} 
                    weight={5} 
                    opacity={0.8}
                  />
                  ...
                  <Marker position={[rota[rota.length - 1].latitude, rota[rota.length - 1].longitude]}>
                    <Popup>Destino</Popup>
                  </Marker>
                </MapContainer>
              ) : (
            ...
            <button className="btn btn-danger" onClick={() => setActivePedidoId(null)}>
